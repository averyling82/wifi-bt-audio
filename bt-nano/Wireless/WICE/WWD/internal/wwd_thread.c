...
static host_thread_type_t    wwd_thread = NULL;
static host_semaphore_type_t wwd_transceive_semaphore = NULL;
...
#else
    RKTaskCreate(TASK_ID_WWD, 0, NULL, SYNC_MODE);
#endif
...
int8_t wwd_thread_send_one_packet( void ) /*@modifies internalState @*/
{
   ...
   // printf("send");
    ...
    {
        //wiced_assert("Could not bring bus back up", 0 != 0 );
        printf("Could not bring bus back up");
        host_buffer_release( tmp_buf_hnd, WWD_NETWORK_TX );
        return 0;
    }
#endif
 ...
 *
 */
extern wiced_buffer_t /*@owned@*/ /*@null@*/ wwd_sdpcm_send_queue_head;
...
COMMON API rk_err_t wwdTask_Suspend(HTC hTask, uint32 Level)
{
    RK_TASK_CLASS*   pWwdTask = (RK_TASK_CLASS*)hTask;

    if(Level == TASK_STATE_IDLE1)
    {
        pWwdTask->State = TASK_STATE_IDLE1;
    }
    else if(Level == TASK_STATE_IDLE2)
    {
        pWwdTask->State = TASK_STATE_IDLE2;
    }
    return RK_SUCCESS;
}

COMMON API rk_err_t wwdTask_Resume(HTC hTask)
{
   RK_TASK_CLASS*   pWwdTask = (RK_TASK_CLASS*)hTask;
   pWwdTask->State = TASK_STATE_WORKING;
   return RK_SUCCESS;
}

COMMON API rk_err_t wwdTask_CheckIdle(HTC hTask)
{
    RK_TASK_CLASS*   pTask = (RK_TASK_CLASS*)hTask;

    if(pTask->State != TASK_STATE_WORKING)
    {
        if(wwdTask_Resume(hTask) != RK_SUCCESS)
        {
            return RK_ERROR;
        }
    }
