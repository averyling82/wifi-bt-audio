...
#define WWD_IOCTL_TIMEOUT_MS         (4000)//(400)
...
/* IOCTL variables*/
static uint16_t                  wwd_sdpcm_requested_ioctl_id = 0;
static host_semaphore_type_t     wwd_sdpcm_ioctl_mutex = NULL;
static /*@only@*/ wiced_buffer_t wwd_sdpcm_ioctl_response = NULL;
static host_semaphore_type_t     wwd_sdpcm_ioctl_sleep = NULL;

/* Bus data credit variables */
 uint8_t wwd_sdpcm_packet_transmit_sequence_number;
 uint8_t wwd_sdpcm_last_bus_data_credit    = 0;
 uint8_t wwd_sdpcm_credit_diff             = 0;
 uint8_t wwd_sdpcm_largest_credit_diff     = 0;

/* QoS related variables */
static uint8_t wwd_sdpcm_highest_rx_tos = 0;

/* Packet send queue variables */
static host_semaphore_type_t                 wwd_sdpcm_send_queue_mutex = NULL;
wiced_buffer_t /*@owned@*/ /*@null@*/ wwd_sdpcm_send_queue_head   = (wiced_buffer_t) NULL;
...
wwd_result_t wwd_sdpcm_send_ioctl( sdpcm_command_type_t type, uint32_t command, /*@only@*/ wiced_buffer_t send_buffer_hnd, /*@special@*/ /*@out@*/ /*@null@*/  wiced_buffer_t* response_buffer_hnd, wwd_interface_t interface )  /*@allocates *response_buffer_hnd@*/  /*@defines **response_buffer_hnd@*/
{
    ...
    WWD_LOG( ( "Wcd:> IOCTL pkt 0x%08X: cmd %d, len %d", (unsigned int)send_buffer_hnd, (int)command, (int)data_length ) );
