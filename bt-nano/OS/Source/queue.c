...
xQueueHandle xReturn = NULL;
    ...
    {
        pxNewQueue = ( xQUEUE * ) pvPortMalloc( sizeof( xQUEUE ), 0);
        if( pxNewQueue != NULL )
        {
            ...
            pxNewQueue->pcHead = ( signed char * ) pvPortMalloc( xQueueSizeInBytes, 1);
...
static void prvCopyDataToQueue( xQUEUE *pxQueue, const void *pvItemToQueue, portBASE_TYPE xPosition )
{
    ...
    {
        if((pxQueue->pcReadFrom == NULL) && (pxQueue->uxItemSize))
        {
            printf("free rtos error = %d\n", pxQueue->uxItemSize);
        }
