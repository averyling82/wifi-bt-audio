...
PRIVILEGED_DATA static xQueueHandle xTimerQueue = NULL;
    ...
    PRIVILEGED_DATA  xTaskHandle xTimerTaskHandle = NULL;
...
portBASE_TYPE xReturn = pdFAIL;
    ...
    if( xTimerQueue != NULL )
    {
        #if ( INCLUDE_xTimerGetTimerDaemonTaskHandle == 1 )
        {
            ...
            xReturn = xTaskCreate( prvTimerTask, ( const signed char * ) "osTimer", ( unsigned short ) configTIMER_TASK_STACK_DEPTH, NULL, ( ( unsigned portBASE_TYPE ) configTIMER_TASK_PRIORITY ) | portPRIVILEGE_BIT, &xTimerTaskHandle,NULL);
        }
        #else
        {
            /* Create the timer task without storing its handle. */
            xReturn = xTaskCreate( prvTimerTask, ( const signed char * ) "osTimer", ( unsigned short ) configTIMER_TASK_STACK_DEPTH, NULL, ( ( unsigned portBASE_TYPE ) configTIMER_TASK_PRIORITY ) | portPRIVILEGE_BIT, NULL, NULL);
        }
        #endif
    }
...
xTIMER *pxNewTimer;
    ...
    {
        pxNewTimer = ( xTIMER * ) pvPortMalloc( sizeof( xTIMER ), 0);
        if( pxNewTimer != NULL )
        {
