...
PRIVILEGED_DATA  xTaskHandle xIdleTaskHandle = NULL;            /*< Holds the handle of the idle task.  The idle task is created automatically when the scheduler is started. */
...
void vTaskStartScheduler( void )
{
...
#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
    {
        ...
        xReturn = xTaskCreate(prvIdleTask, ( signed char * ) "idle", tskIDLE_STACK_SIZE, ( void * ) NULL, (tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle, &Image$$IDLE_STACK$$RW$$Base);
    }
#else
    {
        /* Create the idle task without storing its handle. */
        xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "idle", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL, &Image$$IDLE_STACK$$RW$$Base);
    }
...
unsigned long vTaskGetRunTime(xTaskHandle xTask)
{
    tskTCB *pxTCB;
    pxTCB = (tskTCB *)xTask;
    if(pxTCB == NULL)
...
static tskTCB *prvAllocateTCBAndStack( unsigned short usStackDepth, portSTACK_TYPE *puxStackBuffer )
{
    ...
    pxNewTCB = ( tskTCB * ) pvPortMalloc( sizeof( tskTCB ), 0);
